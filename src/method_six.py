...
from utils import forward_pass, get_log_acceptance
...
def compute_log_joint(N: np.ndarray, C: np.ndarray, lam: int, p: float, S: int) -> float:
    log_lam = stats.gamma.logpdf(lam, S, scale=2)
    log_p = stats.beta.logpdf(p, 1, 1)
    log_C = np.zeros(C.shape)
    for t in range(C.shape[1]):
        log_C[:, t] = stats.binom.logpmf(C[:, t], N, p)
    log_N = stats.poisson.logpmf(N, lam)
    joint = np.sum(log_N + np.sum(log_C, axis=1), axis=0) + log_lam + log_p
    return joint

def run_method_six(sites, T, lam, p, S, EPOCHS, random_state=42) -> None:
    """Gamma prior on lambda and Beta prior on p"""
    print("\n\nRunning method six")
    ...
    save_samples("../data/results", method=6,
    ...
    save_summary("../data/results", method=6,
                 sites=sites, T=T, S=S, EPOCHS=EPOCHS,
