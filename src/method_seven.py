...
def run_method_seven(sites, T, lam, p, S, EPOCHS, random_state=42) -> None:
    """Random walk method w/ truncated normal COMPONENT-WISE (method 5 reimplemented)."""
    print("\n\nRunning method seven: Random walk method w/ truncated normal COMPONENT-WISE (method 5 reimplemented).")
    ...
    for i in range(EPOCHS):
        ...
        for _ in range(10):
            ...
            log_new_joint = compute_log_joint(N, C, new_lam, p, S)
        ...
        for _ in range(10): 
            ...
            log_new_joint = compute_log_joint(N, C, lam, new_p, S)
            acceptance_score = get_log_acceptance(log_old_joint, log_new_joint, log_trans_new_to_old, log_trans_old_to_new)
            ...
            for _ in range(10):
                ...
                while True:
                    ...
                    if proposed >= C_max[j]:
                        ...
                        break
                # Handle N transition
                log_trans_old_to_new = log_truncated_poisson_pmf(new_N[j], mu=N[j], lower=C_max[j])
                log_trans_new_to_old = log_truncated_poisson_pmf(N[j], mu=new_N[j], lower=C_max[j])

                log_new_joint = compute_log_joint(new_N, C, lam, p, S)
    ...
    save_samples("../data/results", method=7,
    ...
    save_summary("../data/results", method=7,
                 sites=sites, T=T, S=S, EPOCHS=EPOCHS,
